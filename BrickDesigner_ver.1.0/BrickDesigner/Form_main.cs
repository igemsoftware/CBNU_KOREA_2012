...
namespace BrickDesigner
{
    public partial class Form_Main : Form
    {
        ...
        public void Add_BrickData(Brick_sub_data bsd)
        {
            ...
            if (isCloning)
            {
                if (MessageBox.Show("클로닝을 취소하겠습니까?", "취소", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
        ...
        private void newToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                // 정보를 저장하기 원하면
                if (MessageBox.Show("현재 정보를 저장하시겠습니까?", "저장", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.OK)
                {
                    // 데이터가 있으면
        ...
        private void paint_Circle()
        {
            if (this.newBD.sl_cloning_list.Count > 0)
            {
                ...
                {
                    if (this.newBD.sl_cloning_list[i].part_seq.Length <= 15)
                    {
                        str_sum += this.newBD.sl_cloning_list[i].part_seq;
                        //i_sum += this.newBD.sl_subpart_list[i].part_seq.Length;
                        i_sum += 10;
                    }
                    else if (this.newBD.sl_cloning_list[i].part_seq.Length <= 500 && this.newBD.sl_cloning_list[i].part_seq.Length > 15)
                    {
                        str_sum += this.newBD.sl_cloning_list[i].part_seq;
                        i_sum += 50;
                    }
        ...
        private void screenShotToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (check_Scar())
            {
                if (this.newBD.sl_subpart_list.Count > 0)
                {
                    // 현재 클로닝이 되어 있다면 클로닝 리스트에서 스샷을 찍는다
            ...
            if (isCloning)
            {
                if (this.newBD.sl_cloning_list.Count > 0)
                {
                    ...
                    for (int i = 0; i < this.newBD.sl_cloning_list.Count; i++)
                    {
                        ...
                        lb.BackColor = Color.FromArgb(_r.Next(0, 255), _r.Next(0, 255), _r.Next(0, 255));
        ...
        private void sBOLToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (this.newBD.sl_subpart_list.Count > 0)
            {
                ...
                sw.WriteLine(this.newBD.str_sub_sequence);
                ...
                if (this.newBD.sl_subpart_list.Count > 0)
                {
                    ...
                    if (saveFD.ShowDialog() == DialogResult.OK)
                    {
                        if ((sr_Stream = saveFD.OpenFile()) != null)
                        {
                            ...
                            //sb_seqContent.Append(this.newBD.str_sub_sequence);
