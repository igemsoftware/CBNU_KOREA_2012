...
namespace BrickDesigner
{
    public partial class Form_Download : Form
    {
        ...
        private void down_fasta_Click(object sender, EventArgs e)
        {
            ...
            if (!downfile.Exists)
            {
                if (MessageBox.Show("Do you want to download files?", "Download", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    bgw.RunWorkerAsync();
                }
            }
            ...
            {
                if (MessageBox.Show("Do you want to re-download files?\n\nPrevious File : " + downfile.LastWriteTime, "Download", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    bgw.RunWorkerAsync();
                }
            }
        ...
        private void BackgroundWorker_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            ...
            if (MessageBox.Show("Adjusting to DB. Estimated time remaining : 1 second(s) - 3 minute(s)", "Database", MessageBoxButtons.OK, MessageBoxIcon.Question) == DialogResult.OK)
            {
                bgw2.RunWorkerAsync();
            }
        }
        ...
        private void button5_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("There isn't a brick you just selected.", "Download", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }

            if (MessageBox.Show("Do you want to download number of " + list_brick.Count + " Bricks?", "Download", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
        ...
        private void Form_download_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (bgw.IsBusy)
            {
                if (MessageBox.Show("Now, downloading files.", "Close", MessageBoxButtons.OK) == DialogResult.OK)
                {
                    e.Cancel = true;
                }
            }
            ...
            {
                MessageBox.Show("Now, processing data.", "Close", MessageBoxButtons.OK);
                e.Cancel = true;
            }
